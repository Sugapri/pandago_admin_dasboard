...
import { allOrders, setAllOrders, orderTableVisible, setOrderTableVisible } from "../store/state.js";
import { numberFormat } from "../utils/format.js";

const STATUS_MAP = {
  pending:     { bg: "bg-amber-100",   text: "text-amber-700",   label: "Menunggu" },
  accepted:    { bg: "bg-blue-100",     text: "text-blue-700",    label: "Diterima" },
  picked_up:   { bg: "bg-indigo-100",   text: "text-indigo-700",  label: "Dijemput" },
  on_the_way:  { bg: "bg-violet-100",   text: "text-violet-700",  label: "Dalam Perjalanan" },
...
function renderOrderRows(orders) {
  ...
  tbody.innerHTML = orders.map((o) => {
    const sc = STATUS_MAP[o.status] || { bg: "bg-slate-100", text: "text-slate-500", label: o.status };
    const priceValue = o.display_price > 0 ? o.display_price : (o.estimated_price || 0);
...
window.resetRevenue = async () => {
  if (!confirm("PERINGATAN: Ini akan menghapus SELURUH data order/pendapatan secara permanen. Anda yakin?")) return;
  try {

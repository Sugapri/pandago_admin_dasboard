...
import {
  ...
  isSanctioned,
} from "../business/drivers.logic.js";
...
function renderVerificationQueue(drivers) {
  ...
  if (badge) {
    if (pendingCount > 0) {
      ...
      badge.className =
        "bg-amber-100 text-amber-700 text-[10px] font-black px-4 py-1.5 rounded-full uppercase";
    } else {
      badge.textContent = "Semua Terverifikasi";
      badge.className =
        "bg-emerald-100 text-emerald-700 text-[10px] font-black px-4 py-1.5 rounded-full uppercase";
    }
  }
  ...
  tbody.innerHTML = queue
    .map((d) => {
      ...
      const avatarBg = isVerified
        ? "bg-emerald-100"
        : isRejected
          ? "bg-rose-100"
          : "bg-amber-100";
      const avatarText = isVerified
        ? "text-emerald-600"
        : isRejected
          ? "text-rose-600"
          : "text-amber-600";
      ...
      const actionButtons = isPending
        ? `<div class="flex justify-center space-x-2">
           ...
           <button onclick="window.openSanctionModal(${d.id}, '${d.name.replace(/'/g, "\\'")}')" class="bg-rose-500/20 text-rose-600 w-8 h-8 rounded-lg hover:bg-rose-500 hover:text-white shadow-md transition active:scale-90" title="Sanksi Driver"><i class="fas fa-gavel text-xs"></i></button>
         </div>`
        : isVerified
          ? `<div class="flex justify-center space-x-2">
          ...
          : `<div class="flex justify-center space-x-2">
             <button onclick="window.actionDriver(${d.id}, 'verified')" class="bg-slate-200 text-slate-500 w-8 h-8 rounded-lg hover:bg-emerald-500 hover:text-white shadow-md transition active:scale-90" title="Verifikasi Ulang"><i class="fas fa-rotate-right text-xs"></i></button>
             <button onclick="window.openSanctionModal(${d.id}, '${d.name.replace(/'/g, "\\'")}')" class="bg-rose-500/20 text-rose-600 w-8 h-8 rounded-lg hover:bg-rose-500 hover:text-white shadow-md transition active:scale-90" title="Sanksi Driver"><i class="fas fa-gavel text-xs"></i></button>
           </div>`;
